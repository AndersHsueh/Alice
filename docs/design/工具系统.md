...
**示例**:
```
用户: "帮我创建一个 Obsidian 笔记"
AI 判断需要加载 obsidian-markdown 技能
工具调用: loadSkill({ skillName: "obsidian-markdown" })
返回: { instructions: "...", success: true }
AI 使用技能指令完成任务
```
...
sequenceDiagram
    ...
    U->>LLM: "读取 package.json"
    
    Note over LLM: 1. 理解意图<br/>需要读取文件
    
    LLM->>LLM: 生成 tool_calls
    ...
    LLM->>LLM: 追加 tool result 到消息
    
    LLM->>LLM: 生成最终回复
    
    LLM-->>U: "你的项目名称是 alice-cli..."
```
