...
> 
> ### 🟡 需要警惕的三个陷阱
> 1. **功能蔓延**：邮件、心跳、多 Agent、Obsidian IPC——每一个都是一个产品。你是一个人（或小团队），不要同时做 5 个产品。**MVP 只做一件事：用 CLI 安全地执行运维操作，带审计日志。** 其他的都是 v1.0+ 的事。
> 2. **竞品对比过于自信**：文档中对 Claude Code 的评价（"❌ 不可审计"、"❌ 不可干预"）是基于当前状态的。大公司迭代速度很快，Claude Code 半年后可能就有审计功能。ALICE 的护城河不能建立在"别人还没做"之上，而要建立在"架构从根基上就是为信任设计的"。
> 3. **成本计算过于乐观**：$171K/年的节省计算假设了 50 次/天的诊断频率和 3 个全职运维的替代。现实中，采购决策者会质疑这些数字。建议用保守估计（10 次/天，节省 1 个人时），反而更可信。
> 
> ### 🔴 我认为的正确优先级
> ```
> v0.3.0 - 核心基础（现在）
