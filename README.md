...
[![Version](https://img.shields.io/badge/version-0.4.1-blue.svg)](https://github.com/AndersHsueh/Alice)
...
**Todo 列表数据层与专用 UI**
- 📋 工具结果支持 `display` 字段，供前端识别并渲染（如 `type: 'todo_list'`）
- 📌 TodoWrite / TodoRead 成功时统一返回 `display.todos`，对话区展示无框、科技蓝风格任务列表（○ pending / ◐ 进行中 / ● 已完成 / ⊘ 已取消）
- 🧩 专用组件 `TodoListDisplay`，ChatArea 在工具摘要下按 `display` 条件挂载
